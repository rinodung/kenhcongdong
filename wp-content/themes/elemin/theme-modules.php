...
/***************************************************************************/	
	...
	function themify_footer_slider($data=array()){
		...
		$slider_ops = array( __('On', 'themify') => 'on', __('Off', 'themify') => 'off' );
		/** Slider status */
		if('' == $data['setting-footer_slider_enabled'] || 'on' == $data['setting-footer_slider_enabled']){
			$enabled_checked = "selected='selected'";
		} else {
			$enabled_display = "style='display:none;'";	
		}
		...
		/** Iterate through slider options */
		foreach ($slider_ops as $key => $val) {
			$output .= '<option value="'.$val.'" ' . selected($data['setting-footer_slider_enabled'], $val, false) . '>' . $key . '</option>';
		}
		$output .= '</select>' . '</p>
					...
					<p class="pushlabel feature_box_posts" '.$display_posts_display.'>';
							$output .= wp_dropdown_categories(array("show_option_all"=>__('All Categories', 'themify'),"hide_empty"=>0,"echo"=>0,"name"=>"setting-footer_slider_posts_category","selected"=>$data['setting-footer_slider_posts_category']));
		...
		$output .= '
			...
			$output .= '<div class="uploader-fields">
							...
							<span class="label">' . __('Image', 'themify') . ' '.$x.'</span>
							<input type="text" class="width10 feature_box_img" id="setting-footer_slider_images_'.$option.'_image" name="setting-footer_slider_images_'.$option.'_image" value="'.$data['setting-footer_slider_images_'.$option.'_image'].'" /> 
