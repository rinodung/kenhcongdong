...
class WPB_Google_Drive_Cdn extends WPB_Plugin {
	...
	/**
	 * The list of excluded files that have wildcards.
	 *
	 * @since  1.8.0
	 * @access public
	 *
	 * @var array $_excluded_files_wildcards The key is the URL.
	 */
	private $_excluded_files_wildcards = null;
	...
	public function menu( $id ) {
		...
		$this->_pagehook = add_options_page( __( 'Google Drive CDN', $this->get_textdomain() ), __( 'Google Drive CDN', $this->get_textdomain() ), 'administrator', 'wpbgdc', array(
			&$this,
			'settings_page'
		) );
	...
	public function settings_page() {
		...
		add_meta_box( 'wpbgdc_enabling_api', __( '1. Enable the Drive API', $this->get_textdomain() ), array(
			$metabox_class,
			'enabling_api'
		), $this->_pagehook, 'normal', 'core' );

		if ( ! empty( $client_id ) && ! empty( $client_secret ) ) {
			add_meta_box( 'wpbgdc_connect', __( '2. Connect with your Google Drive', $this->get_textdomain() ), array(
				$metabox_class,
				'connect'
		...
		if ( $is_api_working ) {
			add_meta_box( 'wpbgdc_info', __( '3. Instructions - READ THIS CAREFULLY', $this->get_textdomain() ), array(
				$metabox_class,
				'info'
			), $this->_pagehook, 'normal', 'low' );
		}
		...
		if ( $is_api_working ) {
			add_meta_box( 'wpbgdc_settings', __( '4. Settings', $this->get_textdomain() ), array(
				$metabox_class,
				'settings_section'
			), $this->_pagehook, 'normal', 'low' );
		}

		add_meta_box( 'wpbgdc_log', __( 'Error Logs, hints and statistics', $this->get_textdomain() ), array(
			$metabox_class,
			'error_logs'
		), $this->_pagehook, 'normal', 'low' );

		// Add metabox "about"
		add_meta_box( 'wpbgdc_about', __( 'About', $this->get_textdomain() ), array(
			$metabox_class,
			'about'
		), $this->_pagehook, 'side', 'default' );

		// Add metabox "links"
		add_meta_box( 'wpbgdc_links', __( 'Helpful links', $this->get_textdomain() ), array(
			$metabox_class,
			'links'
		), $this->_pagehook, 'side', 'default' );

		// Add metabox "social"
		add_meta_box( 'wpbgdc_social', __( 'Like this plugin?', $this->get_textdomain() ), array(
			$metabox_class,
			'social'
		), $this->_pagehook, 'side', 'default' );

		// Add metabox "subscribe"
		add_meta_box( 'wpbgdc_subscribe', __( 'Get our free Newsletter', $this->get_textdomain() ), array(
			$metabox_class,
			'subscribe'
		), $this->_pagehook, 'side', 'default' );
	...
	public function buffer( $buffer ) {
		...
		if ( $this->_is_buffer_css_file ) {
			...
			$buffer = preg_replace_callback( '#(@import){1}(\s)*(url)?(\s)*\(?(\s)*("|\')?([A-Za-z0-9-.:\/]*)?("|\')?(\s)*\)?(\s)*([A-Za-z0-9,\s]*)?;#is', array(
				&$this,
				'collect_css_file_imports'
			), $buffer );

			return preg_replace_callback( '#url\((.+?)\)#is', array( &$this, 'collect_css_file_urls' ), $buffer );
		}

		return preg_replace_callback( '#(?<=(\'|"))(https?://[\d\w\-\./]+\.(?:' . implode( '|', $file_extensions ) . ')(?!\.))#is', array(
			&$this,
			'collect_urls'
		), $buffer );
	}
	...
	/**
	 * Checks if the current URL is in a wildcard
	 * @since  1.8.0
	 * @access public
	 *
	 * @param string $url
	 ...
	 * @return bool
	 */
	public function is_file_excluded( $url ) {

		# check if the URL is in the list of excluded files
		if ( isset( $this->_excluded_files[ $url ] ) ) {
			return true;
		}

		# check if there are any wildcard URLs
		if ( is_null( $this->_excluded_files_wildcards ) ) {
			$this->_excluded_files_wildcards = array();
			foreach ( $this->_excluded_files as $url => $empty ) {
				if ( false !== strpos( $url, '*' ) ) {
					$this->_excluded_files_wildcards[] = '/' . str_replace( '*', '.*', preg_quote( $url, '/' ) ) . '/is';
				}
			}
		}

		if ( empty( $this->_excluded_files_wildcards ) ) {
			return false;
		}

		foreach ( $this->_excluded_files_wildcards as $wc_pattern ) {
			if ( 1 === @preg_match( $wc_pattern, $url ) ) {
				return true;
			}
		}
	...
	public function on_deactivation() {
		...
		$sql = "DROP TABLE IF EXISTS `" . $wpdb->prefix . "wpbgdc_files`;";
	...
	public function ajax_sync() {
		...
		foreach ( $attachments as $attachment ) {
			...
			$file = str_replace( $content_url, $upload_dir, $attachment->guid );
	...
	public function add_schedule_time( $times ) {
		...
		$times['1week']  = array(
			...
			'display'  => __( 'Once a week', $this->get_textdomain() )
		);
		$times['2weeks'] = array(
			'interval' => 2 * WEEK_IN_SECONDS,
			'display'  => __( 'Every 2th week', $this->get_textdomain() )
		);
		$times['3weeks'] = array(
			'interval' => 3 * WEEK_IN_SECONDS,
			'display'  => __( 'Every 3rd week', $this->get_textdomain() )
		);
		$times['1month'] = array(
			'interval' => 4 * WEEK_IN_SECONDS,
			'display'  => __( 'Once a month', $this->get_textdomain() )
		);
