...
class Chat {
	...
	public static function send_contact_from( $data ) {
		...
		if( empty( $data['f_chat_user_name'] ) or empty( $data['f_chat_user_email'] ) or empty( $data['f_chat_user_question'] ) or ( empty( $data['f_chat_user_phone'] ) and $sc_opts['ask_phone_field'] == 1 ) ) {
		...
		$subject = '[' . get_bloginfo( 'name' ) . '] ' . sprintf( __( '%s wrote a note', 'sc_chat' ), $data['f_chat_user_email'] );
